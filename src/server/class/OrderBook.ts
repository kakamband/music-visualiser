...
export default class OrderBook {
  ...
  public constructor(
    depth = 1000,
    tPair: TradePair = { base: 'BTC', quote: 'USDT' }
  ) {
    tPair = { base: tPair.base.trim(), quote: tPair.quote.trim() };
    const snapshotURL = `https://www.binance.com/api/v1/depth?symbol=${
      tPair.base + tPair.quote
    ...
    axios
      ...
      .catch((error: any) => {
        process.stderr.write(
          `error in retrieving orderbook snapshot from binance: ${error.response.data.msg}\n`
        );
  ...
  /* user functions */
  public updateOrderBook(type: OrderType, target: [number, number]): void {
    /* updation through binary search */

    const prices: [number, number][] =
      type === OrderType.buy ? this.buyers : this.sellers;
    const binIndex: number = this.binarySearch(
      type,
      0,
      prices.length - 1,
      target[0],
    ...
    if (prices.length !== OrderBook.EMPTY_BOOK) {
      ...
      if (target[PRICE] === prices[binIndex][PRICE]) {
        /* if found */
        target[QUANT] === OrderBook.DELETE_ENTRY
          ? prices.splice(binIndex, 1)
          : (prices[binIndex][QUANT] =
              target[QUANT]); /* if true delete, else update */
      } else if (target[QUANT] !== OrderBook.DELETE_ENTRY) {
        /* not found, insert */
        switch (type) {
          case OrderType.buy /* highest price to lowest price - buyers */:
            target[PRICE] > prices[binIndex][PRICE]
      ...
      /* empty order book */
      if (target[QUANT] !== OrderBook.DELETE_ENTRY) {
  ...
  private parseBinanceOrderBookSnapshot(res: any) {
    ...
    for (let i = 0; i < res.data.bids.length; i++) {
      ...
      const newEntry: [number, number] = [
        parseFloat(res.data.bids[i][PRICE]),
        parseFloat(res.data.bids[i][QUANT])
      ];
      this.buyers.push(newEntry);
    }
    for (let i = 0; i < res.data.asks.length; i++) {
      const newEntry: [number, number] = [
        parseFloat(res.data.asks[i][PRICE]),
        parseFloat(res.data.asks[i][QUANT])
      ];
    ...
    console.log(
      this
    ); /* change to process.stdout.write() when it is possible */
  }
  ...
  private binarySearch(
    ...
    */
    while (first <= last) {
      ...
      const movePriceEntryToEnd: boolean =
        type === OrderType.buy
          ? this.buyers[mid][PRICE] > targetPrice
          : this.sellers[mid][PRICE] < targetPrice;
      const movePriceEntryToStart: boolean =
        type === OrderType.buy
          ? this.buyers[mid][PRICE] < targetPrice
          : this.sellers[mid][PRICE] > targetPrice;
