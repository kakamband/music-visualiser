...
class OrderBook {
    ...
    constructor(depth = 1000, tPair = { base: 'BTC', quote: 'USDT' }) {
        ...
        axios_1.default.get(snapshotURL) /* gets snapshot data */
    ...
    updateOrderBook(type, target) {
        ...
        if (prices.length !== OrderBook.EMPTY_BOOK) { /* if local order book is not empty */
            if (target[helperTypes_1.PRICE] === prices[binIndex][helperTypes_1.PRICE]) { /* if found */
                (target[helperTypes_1.QUANT] === OrderBook.DELETE_ENTRY) ? prices.splice(binIndex, 1) : prices[binIndex][helperTypes_1.QUANT] = target[helperTypes_1.QUANT]; /* if true delete, else update */
            }
            else if (target[helperTypes_1.QUANT] !== OrderBook.DELETE_ENTRY) { /* not found, insert */
                switch (type) {
                    case helperTypes_1.OrderType.buy: /* highest price to lowest price - buyers */
                        (target[helperTypes_1.PRICE] > prices[binIndex][helperTypes_1.PRICE]) ? prices.splice(binIndex, 0, target) : prices.splice(binIndex + 1, 0, target);
                        break;
            ...
            if (target[helperTypes_1.QUANT] !== OrderBook.DELETE_ENTRY) { /* add to order book if not delete entry */
                prices.push(target);
            }
        }
    }
    ...
    parseBinanceOrderBookSnapshot(res) {
        ...
        for (let i = 0; i < res.data.bids.length; i++) { /*design choice to seperate buyers and sellers.
            ...
            const newEntry = [parseFloat(res.data.bids[i][helperTypes_1.PRICE]), parseFloat(res.data.bids[i][helperTypes_1.QUANT])];
            this.buyers.push(newEntry);
        }
        for (let i = 0; i < res.data.asks.length; i++) {
            const newEntry = [parseFloat(res.data.asks[i][helperTypes_1.PRICE]), parseFloat(res.data.asks[i][helperTypes_1.QUANT])];
    ...
    binarySearch(type, first, last, targetPrice, prices = []) {
        ...
        */
        while (first <= last) {
            ...
            const movePriceEntryToEnd = (type === helperTypes_1.OrderType.buy) ? (this.buyers[mid][helperTypes_1.PRICE] > targetPrice) : (this.sellers[mid][helperTypes_1.PRICE] < targetPrice);
            const movePriceEntryToStart = (type === helperTypes_1.OrderType.buy) ? (this.buyers[mid][helperTypes_1.PRICE] < targetPrice) : (this.sellers[mid][helperTypes_1.PRICE] > targetPrice);
