 ...
 */
const test: TestInterface<CustomInterface<
  bigint
>> = orginalTestImport as TestInterface<CustomInterface<bigint>>;
...
test('Does Linked list add correctly?', (t: ExecutionContext<
  CustomInterface<bigint>
>) => {
  ...
  t.is(
    t.context.linkedList.elementCount,
    t.context.data.length
  ); /* Note the element count is accessed like it's a property (when it is in fact a function) */
});
